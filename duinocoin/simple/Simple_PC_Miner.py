    ...
    if not Path(RESOURCES_DIR + "/Miner_config.cfg").is_file():
        ...
        if xxhash_enabled:
            ...
            print(
                ...
                + " - XXHASH")
            algorithm = input(
                "Select mining algorithm you want to use (1-2): ")
        else:
            algorithm = "1"
        ...
        requested_diff = input(
            "Select mining difficulty you want to use (1-3): ")
        ...
        if threadcount == "":
            ...
            threadcount = 8
            print("Mining Duino-Coin with a lot of threads will cause decrease in profit - mining threads decreased to 8")
...
def Thread(
    ...
    while True:
        ...
        if algorithm == "XXHASH":
            using_algo = " using XXHASH algorithm with "
        else:
        ...
        while True:
            try:
                ...
                while True:
                    ...
                    if algorithm == "XXHASH":
                        algo_back_color = "XXHASH"
                        result = ducos1xxh(job[0], job[1], diff, efficiency)
                    else:
                        algo_back_color = "DUCOS1"
                    ...
                    while True:
                        ...
                        if feedback == "GOOD":
                            ...
                            accepted.value += 1
                            title(
                                "Simple Duino-Coin Python Miner (v"
                                + str(MINER_VER)
                                + ") -"
                                + str(accepted.value)
                                + "/"
...
if __name__ == "__main__":
    ...
    title("Simple_PC_miner by gsrbr based on Official Duino-Coin Python Miner" + str(MINER_VER) + ")")

    if osname == "nt":
        # Unicode fix for windows
        ossystem("chcp 65001")
    print("Hello, welcome to the Simple miner, this it's an adapted version of the Official PC miner, without any colorama dependence or special characters.\nWarning\nAs it is an adapted version, only for users of old systems, this miner may not receive constant updates or even not receive updates\nthanks for understanding \n-gsrbr - https://github.com/gsrbr")
    sleep(3)
